...
import { GetManualSamityApplicationData } from "../../../services/coop/application/getManualSamityApplicationData.service";
...
router.get(
  ...
  wrap(async (req: Request, res: Response, next: NextFunction) => {
    ...
    } else if (req.query.samityId && req.query.type === "migrationCorrectionApplicationData") {
      // *********************** add by Md. Hasibuzzaman *******************
      const ServiceInfoService = Container.get(ServiceInfoServices);
      const serviceId = await ServiceInfoService.getServiceByNameAndDoptor(
        "samity_migration_correction",
        req.user.doptorId
      );
      const migSamityInfo = Container.get(GetManualSamityApplicationData);
      applicationData = await migSamityInfo.getMigrationApplicationById(
        req.query.samityId,
        req.user.doptorId,
        serviceId
      );
...
router.post(
  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      // *************************** audit Accounts part by Hasibuzzaman *************************
      if (type == "samity-migration-correction") {
        const SamityMigrationService = Container.get(SamityMigrationServices);
        data = await SamityMigrationService.manualSamityMigrationCorrection(req.body.data, req.user);
        d = { id: req.body.nextAppDesignationId };
        createdBy;
      }
