...
export const manualSamityMigrationValidation = [
  ...
  body("data.memberInfo", "অথরাইজড / অনুমোদিত ব্যক্তির তথ্য ফাকা হতে পারে না")
    .optional()
    .custom(async (value, { req }) => {
      if (
        (req.body.data.samityInfo.samityLevel == "P" || req.body.data.samityInfo.samityLevel == "C") &&
        req.body.data.samityInfo.samityEffectiveness === "A"
      ) {
        if (!value) {
  ...
  body("data.memberInfo.memberNameBangla")
    .optional()
    .custom((value: any, { req }: Meta) => {
      if (
        (req.body.data.samityInfo.samityLevel == "P" || req.body.data.samityInfo.samityLevel == "C") &&
        req.body.data.samityInfo.samityEffectiveness === "A"
      ) {
...
export const manualSamityMigrationCorrectionRequest = [
  ...manualSamityMigrationValidation,
  ...documentValidation
];
export const manualSamityMigrationCorrectionUpdate = [
  ...manualSamityMigrationValidation,
  body("data.documentInfo")
    .exists()
    .withMessage("document Info is not present in payload")
  ...
  // body("data.documentInfo.*.documentName.*.mimeType")   *** pore thik kora lagbe
  //   .exists()
  //   .withMessage(" ডকুমেন্ট এর ছবি নির্বাচন করুন ")
  //   .notEmpty()
  //   .withMessage(" ডকুমেন্ট এর ছবি নির্বাচন করুন")
