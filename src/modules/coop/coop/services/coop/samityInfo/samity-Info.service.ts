...
export class SamityInfoServices {
  ...
  async getBySamityId(samityId: number) {
    ...
    try {
      ...
      const queryTextFor = `SELECT * FROM coop.samity_document WHERE samity_id=$1`;
      const samityDocument = (await transaction.query(queryTextFor, [samityId])).rows;

      const queryForAuthorizedPerson = `
      SELECT A.MEMBER_NAME_BANGLA AS AUTHORIZEDPERSONNAME,
      A.NID AS AUTHORIZEDPERSONNID,
      A.MOBILE AS AUTHORIZEDPERSONMOBILE,
      B.SAMITY_ID
      FROM COOP.MEMBER_INFO A
      RIGHT JOIN COOP.SAMITY_AUTHORIZED_PERSON B ON A.ID = B.MEMBER_ID
      WHERE A.SAMITY_ID = $1`;

      const authorizedPerson = (await transaction.query(queryForAuthorizedPerson, [samityId])).rows[0];
      ...
      return {
        ...
        SamityDocument: samityDocument ? await minioPresignedGet(samityDocument, ["document_name"]) : [],
        authorizedPerson: authorizedPerson ? toCamelKeys(authorizedPerson) : {},
      };
    } catch (ex) {
    } finally {
      transaction.release();
