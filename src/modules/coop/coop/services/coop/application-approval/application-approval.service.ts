...
export class ApplicationApprovalServices {
  ...
  async create(
    ...
    try {
      ...
      // ************************ Manual samity correction Development by Hasibuzzman *************
      if (serviceAction.applicationStatus === "A" && serviceId === 18 && serviceAction.isFinalAction == true) {
        const createdBy = user.userId;
        const createdAt = new Date();
        const sql = `SELECT
                         doptor_id,
        ...
        const workingArea = applicationData.data.workingArea;
        const samityInfo = applicationData.data.samityInfo;
        const memberInfo = applicationData.data.memberInfo;
        console.log({ isMemberAreaAndWorkingAreaSame: samityInfo.isMemberAreaAndWorkingAreaSame });
        const updateSamityInfo = {
          samityName: samityInfo.samityName,
          samityLevel: samityInfo.samityLevel,
          organizerId: samityInfo.organizerId,
          enterprisingId: samityInfo.enterprisingId,
          projectId: samityInfo.projectId,
          officeId: samityInfo.officeId,
          samityDivisionId: samityInfo.samityDivisionId,
          samityDistrictId: samityInfo.samityDistrictId,
          samityUpaCityId: samityInfo.samityUpaCityId,
          samityUpaCityType: samityInfo.samityUpaCityType,
          samityUniThanaPawId: samityInfo.samityUniThanaPawId,
          samityUniThanaPawType: samityInfo.samityUniThanaPawType,
          samityDetailsAddress: samityInfo.samityDetailsAddress,
          samityTypeId: samityInfo.samityTypeId,
          memberAreaType: samityInfo.memberAreaType,
          samityFormationDate: new Date(samityInfo.samityFormationDate),
          samityRegistrationDate: new Date(samityInfo.samityRegistrationDate),
          oldRegistrationNo: samityInfo.oldRegistrationNo,
          workingAreaType: samityInfo.workingAreaType,
          samityEffectiveness: samityInfo.samityEffectiveness,
        };
        //******************* */ update samity table **********************
        const { sql: samityUpdateSql, params: samityUpdateParams } = buildUpdateWithWhereSql(
          "coop.samity_info",
        ...
        //********************** Update member info ***************************
        const memberSql = `select member_id from coop.samity_authorized_person where samity_id=$1`;
        const memberId = (await transaction.query(memberSql, [samityId])).rows[0];
        if (samityInfo.samityLevel == "P") {
          const memberData = {
            nid: memberInfo.nid,
        ...
        //********************** update memberArea Table **********************
        for (let memberElement of memberArea) {
          memberElement = _.omit(memberElement, [
            "districtNameBangla",
            "divisionNameBangla",
            "upaCityNameBangla",
        ...
        ///////////////////////// update workingArea table ////////////////////////
        if (samityInfo.isMemberAreaAndWorkingAreaSame) {
          //need member area data insert this working area
          const sql = `DELETE FROM coop.working_area WHERE samity_id=$1`;
          const pool = await pgConnect.getConnection();
          await pool.query(sql, [samityId]);
          // ***********************************
          // samity_id: 974,
          // division_id: 2451,
          // district_id: 2569,
          // upa_city_id: 2640,
          // upa_city_type: 'UPA',
          // uni_thana_paw_id: 2641,
          // uni_thana_paw_type: 'UNI',
          // details_address: 'Notun Bazar notun para',
          // status: 'a',
          // created_by: '145',
          // created_at: 2023-11-20T10:45:49.302Z,
          // updated_by: '100',
          // updated_at: 2023-11-23T12:04:09.555Z
    

          // get member area data
          const memberSql = `select * from coop.member_area where samity_id=$1`;
          const memberAreaData = (await transaction.query(memberSql, [samityId])).rows;
          for (let memberElement of memberAreaData) {
            memberElement = _.omit(memberElement, ["id"]);
            const { sql: workingInsertSql, params: workingInsertParams } = buildInsertSql("coop.working_area", {
              ...toCamelKeys(memberElement)
            });
            await transaction.query(workingInsertSql, workingInsertParams);
          }
        } else {
          for (let workingElement of workingArea) {
            workingElement = _.omit(workingElement, [
              "districtNameBangla",
              "divisionNameBangla",
              "upaCityNameBangla",
              "uniThanaPawNameBangla",
        ...
        // update documentList table
        applicationData.data.documentInfo = [
          ...applicationData.data.documentInfo.map((elements: any) => {
            return {
              samityId,
              documentId: elements.documentId,
              documentName: elements.documentName[0]?.fileName,
              isCreate: elements.documentName[0]?.isCreate,
              id: elements.id,
              documentNo: elements.documentNo,
              createdAt,
              createdBy,
            };
          }),
        ...
        for (let docElement of applicationData.data.documentInfo) {
          let bdData;
          if (docElement?.id && docElement?.isCreate) {
            docElement = _.omit(docElement, ["isCreate"]);
            const { sql: documentSql, params: documentParams } = buildUpdateWithWhereSql(
              "coop.samity_document",
              { id: docElement.id },
              docElement
            );
            bdData = (await transaction.query(documentSql, documentParams)).rows;
          } else if (!docElement?.id && docElement?.isCreate) {
            docElement = _.omit(docElement, ["isCreate", "id"]);
            const { sql: documentSql, params: documentParams } = buildInsertSql("coop.samity_document", docElement);
            bdData = (await transaction.query(documentSql, documentParams)).rows;
          }
          console.log({ finalResult: bdData });
        }
      }
