...
import { GetManualSamityApplicationData } from "./coop/application/getManualSamityApplicationData.service";
...
export class ApplicationServices {
  ...
  async update(type: string, id: number, reqBody: ApplicationAttrs, user: UsersTokenAttrs) {
    ...
    } else if (type === "samity-migration-correction") {
      console.log("working___________________________");
      const GetManualSamityApplicationDatas = Container.get(GetManualSamityApplicationData);
      result = await GetManualSamityApplicationDatas.update(id, reqBody, user, updatedBy);
    } else {
      const { sql, params } = buildUpdateWithWhereSql(
        "coop.application",
        {
