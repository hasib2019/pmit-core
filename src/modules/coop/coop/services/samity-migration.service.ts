...
export class SamityMigrationServices {
  ...
  async manualSamityMigrationCreate(data: any, user: any) {
    data.officeId = user.officeId;
    data.createdAt = new Date();
  ...
  // added Hasibuzzaman manual samity Migration correction
  async manualSamityMigrationCorrection(data: any, user: any) {
    data.officeId = user.officeId;
    data.createdAt = new Date();
    ...
    for (const element of data.documentInfo) {
      let attachment = [];
      for (let elementDocumentName of element.documentName) {
        console.log({ elementDocumentName });
