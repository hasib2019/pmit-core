...
export class ApplicationServices {
  ...
  async create(payload: any, next: NextFunction): Promise<ApplicationAttrs> {
    ...
    //create loan product application
    ...
    //update loan product application
    ...
    // inventory  inventoryItemRequisition
      ...
      // throw new BadRequestError("Please Stop");
      let { sql, params } = buildInsertSql("temps.application", {
        ...lodash.omit(payload, ["officeId", "serviceName"]),
      });
      result = await (await pool.query(sql, params)).rows[0];
    }
    // inventory  inventoryItemReturn
    ...
    // dps/Fdr Migration
    else if (payload?.serviceName === "dpsFdrMigration") {
      const samityMigrationService = Container.get(SamityMigrationService);
      // try {
      //   const migrationResult = await samityMigrationService.storeLoanInfo(payload);
      //   result = migrationResult;
    ...
    const service = await this.getServiceById(payload.serviceId as number);
    console.log({ service });
    ...
    if (service && service.notification) {
      ...
      await notification.create(service.notification as notificationObject, {
        ...
        doptorId: payload.doptorId,
        componentId: payload.componentId,
        serviceId: payload.serviceId as number,
        applicationId: result.id,
        message: await notification.createCustomNotificationMessageForLoan(payload.serviceId as number, result.id),
        createdBy: payload.createdBy,
      });
    }
