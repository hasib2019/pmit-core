      ...
      if (serviceAction.isFinal && serviceAction.applicationStatus == "R") {
        ...
        if (serviceId == 22 && (+data?.serviceActionId === 3 || +data?.serviceActionId === 4)) {
          if (data?.payload) {
            ...
            let applicationInfo = (await transaction.query(applicationGetSql, [data?.applicationId])).rows[0].data;
            applicationInfo = applicationInfo ? toCamelKeys(applicationInfo) : {};
            const payload = JSON.parse(data.payload);
            console.log("payloadOfService22", payload);
        ...
        if (serviceAction.isFinal) {
          ...
          await notificationService.create(serviceAction.notification as notificationObject, {
            ...
            // forwardId: data.nextAppDesignationId,
            serviceId,
            componentId: 1,
