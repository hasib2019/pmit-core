...
export function dynamicValidates(validators: ValidationChain[], allowExtraFields: Boolean = true) {
  return async (req: Request, res: Response, next: NextFunction) => {
    const type: string = req.params.type as string;
    return validates([...validators, ...applicationValidation(type)], allowExtraFields)(req, res, next);
  };
}
