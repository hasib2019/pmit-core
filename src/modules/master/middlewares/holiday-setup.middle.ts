...
const holidaySetupMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  
  const holidaySetUpService: HolidayInfoServices =
    Container.get(HolidayInfoServices);
  const isDateRangeHaveHoliday =
    await holidaySetUpService.chekGivenDateRangeHaveWeekendDay(
      req.body.fromDate,
      req.body.toDate,
  ...
  if (req.body.holidayType === "WEK1" || req.body.holidayType === "WEK2") {
    if (isDateRangeHaveHoliday === false) {
      next(
        new BadRequestError(
          "আপনার প্রদত্ত তারিখ সীমার মধ্যে শুক্রবার অথবা শনিবার নেই"
        )
      );
    } else {
      next();
