...
@Service()
export default class LoanInfoScheduleServices {
  count: number;
  constructor() {
    this.count = 1;
  }
  ...
  async generateSchedule() {
    ...
    for (const element of scheduleJobList) {
      ...
      try {
        const archiveLogData = (await pgConnect.getConnection().query(archiveLogSql, aechiveLogParams)).rows;
      } catch (ex: any) {
        console.log(ex);
      }

      //   console.log("archiveLogData", archiveLogData);
