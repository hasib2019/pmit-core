...
export class ScheduleCalculator extends ServiceChargeCalculator {
  ...
  async getNextScheduleDate(dateCursor: Moment | undefined, startDate: Moment): Promise<string | null> {
    ...
    if (this.installmentType == "Q" && !this.weekPosition) {
      dateCursor == startDate ? startDate : dateCursor.add(3, "month");
  ...
  getFirstScheduleDate() {
    ...
    //for one time loan
    if (this.installmentType === "O") {
      console.log({ firstDate });
      firstDate.add(12, "month");
      console.log({ firstDate });
      return firstDate;
    }

    // for quarterly loan
    if (this.installmentType === "Q") {
      firstDate.add(3, "month");
      return firstDate;
    }
