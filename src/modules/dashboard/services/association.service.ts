...
export class AssociationSyncService extends Dashboard {
  ...
  async sendSamityToDashboard(
    ...
    try {
      const dashboardToken = await this.getConfig();
      for await (const samity of samityData) {
        try {
          const res = await axios.post(`${dashboardUrl}/api/v1/associations`, samity, dashboardToken);
          if (res.status == 200) {
          ...
          let { sql: successLogSql, params: successLogParams } = buildInsertSql("log.dashboard_api_log", {
            requestNum,
          ...
          let successLog = (await logDbConnection.query(successLogSql, successLogParams)).rows[0];
        } catch (error: any) {
          let { sql: errorLogSql, params: errorLogParams } = buildInsertSql("log.dashboard_api_log", {
            requestNum,
            requestDate: new Date(),
            requestType: "send_samity_to_dashboard",
            userId: userId,
          ...
          let errorLog = (await logDbConnection.query(errorLogSql, errorLogParams)).rows[0];
        }
      }
