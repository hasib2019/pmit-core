...
router.get(
  ...
  wrap(async (req: Request<{ component: ComponentType }>, res: Response, next: NextFunction) => {
    ...
    let data: any = {};
    const doptorService = new DoptorSyncService(req.params.component);
    const token = await doptorService.getConfig();
    console.log({ token });

    if (doptorId && !officeId) {
      data = await doptorService.syncData(token, Number(doptorId));
    } else if (doptorId && officeId) {
      data["officeCount"] = await doptorService.syncOfficeInfo(Number(doptorId), token, Number(officeId));
      data["officeMaterializedViewRefreshStaus"] = await doptorService.refreshOfficeMaterializedView();

      data = { ...data, ...(await doptorService.syncByOffice(Number(doptorId), token, Number(officeId))) };
    } else {
      data = await doptorService.syncData(token);
    }
